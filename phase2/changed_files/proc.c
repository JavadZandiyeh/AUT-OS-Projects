...
growproc(int n)
{
  ...
  if(curproc->threads == -1) // child
  {
    ...
    // -2 is because of update parent along with one child
...
fork(void)
{
  ...
  // there is only one thread for the child because in fork, fork creates a new pgdir for the child process
  np->threads = 1;
  np->parent = curproc;
  *np->tf = *curproc->tf;
...
thread_create(void *stack)
{
  ...
  // Remember stack grows downwards Thus the stackTop will be in the address given be parent
  ...
  // copying all trapframe register values from curproc(parent) into np(child)
  *np->tf = *curproc->tf;
...
thread_wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc)
        continue;
      if(p->threads != -1) // remember thread_wait only waits for threads not child processes
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
